...
type ProductWithPrimaryImage = TProduct & {
  primaryImage?: string;
};

type Props = { product: ProductWithPrimaryImage };
...
export default function ProductZoomSimple({ product }: Props) {
  ...
  const safeIndex =
    allImages.length === 0 ? -1 : Math.min(activeIndex, allImages.length - 1);

  const activeImg = safeIndex >= 0 ? allImages[safeIndex] : "";
    ...
    <div>
      ...
      <div className="flex gap-2 mt-3">
        {(allImages.length ? allImages : [""]).slice(0, 8).map((src, idx) => (
          <button
            key={`${src}-${idx}`}
            onClick={() => src && setActiveIndex(idx)}
            className={`w-[72px] h-[72px] border bg-white overflow-hidden rounded-md ${
              idx === safeIndex ? "border-black border-2" : "border-gray-200"
            }`}
            type="button"
          >
            {src ? (
